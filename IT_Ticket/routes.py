...
def register_routes(app):
    ...
    def approvals():
        if not current_user.is_manager():
            abort(403)
        # Mostra sia i ticket in attesa di approvazione generica che quelli in attesa di approvazione IT
        pending_tickets = Ticket.query.filter(Ticket.status.in_([
            TicketStatus.AWAITING_APPROVAL,
            TicketStatus.AWAITING_IT_MANAGER_APPROVAL
        ])).order_by(Ticket.created_at).all()
        # Ottieni la lista degli utenti IT attivi per il menu a tendina di assegnazione
        it_users = User.query.filter_by(role=UserRole.IT, is_active=True).all()
        return render_template('manager/approvals.html', tickets=pending_tickets, it_users=it_users)
